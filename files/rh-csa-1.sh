...
# common checks
    ...
    printf "Checking sshd configs....\t\t"; sleep 2; [[ $(sudo egrep -i "^PasswordAuthentication no$" /etc/ssh/sshd_config) ]] && [[ $(sudo egrep -i "^PubKeyAuthentication yes$" /etc/ssh/sshd_config) ]] && $success  || $failure
